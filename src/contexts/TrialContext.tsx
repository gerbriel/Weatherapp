...
interface TrialLocation {
  ...
  weatherstationID?: string; // Alias for cimisStationId (used by LocationAddModal)
  weatherstation?: string;   // Optional weather station name
}
...
export const TrialProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addLocation = (locationData: Omit<TrialLocation, 'id'>) => {
    const newLocation: TrialLocation = {
      ...locationData,
      // Normalize weatherstationID → cimisStationId so CMIS lookup always works
      cimisStationId: locationData.cimisStationId || locationData.weatherstationID,
