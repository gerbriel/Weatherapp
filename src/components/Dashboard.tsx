...
export const Dashboard: React.FC = () => {
  ...
  // Wrapper to debug state updates
  const handleCropWeeklySummariesChange = (newSummaries: Record<string, string>) => {
    console.log('🔴 Dashboard received new summaries:', JSON.stringify(newSummaries));
    setCropWeeklySummaries(newSummaries);
    console.log('🔴 Dashboard state setter called');
  };
  ...
  useEffect(() => {
    // Only run if we have instances and haven't cleaned yet
    if (cropInstances.length === 0 || hasCleanedKcRef.current) {
      return;
    }
    ...
    console.log('Cleaning up custom Kc values that match defaults...');
    ...
    setCropInstances(prevInstances => 
      prevInstances.map(instance => {
        ...
        if (removedCount > 0) {
          console.log(`Cleaned ${removedCount} default Kc values from ${crop.name}`);
        }
                        ...
                        <div className="text-center">
                          <div className="text-2xl font-bold text-green-400">
                            {(() => {
                              ...
                              return weatherData.daily.et0_fao_evapotranspiration[0]?.toFixed(3);
                            })()}
                          </div>
                  ...
                  <div className="mb-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {getLocationCropInstances().map((instance) => {
                        ...
                        const et0Value = weatherDataForCrop?.daily?.et0_fao_evapotranspiration?.[0] || 0.2; // API already returns in inches
                            ...
                            <div className="bg-gray-50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-600 rounded-lg p-3 mb-3">
                              ...
                              <div className="grid grid-cols-3 gap-3 mb-2">
                                ...
                                <div>
                                  ...
                                  <p className="text-lg font-bold text-green-400">{etcInches.toFixed(3)}</p>
                                </div>
                              </div>
                              <div className="space-y-1">
                                ...
                                <div className="text-xs text-gray-600 dark:text-gray-300 font-mono">
                                  {etcInches.toFixed(3)} = {et0Value.toFixed(3)} × {displayKc.toFixed(2)}
                                </div>
                              </div>
                            </div>
  ...
  "update_frequency": "daily"
              ...
              <>
                ...
                <ReportView 
                  ...
                  onCropWeeklySummariesChange={handleCropWeeklySummariesChange}
                />
              </>
            ) : currentView === 'notifications' ? (
              <>
